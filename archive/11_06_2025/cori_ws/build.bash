...
URDF_FILE="src/cori_description/urdf/cori.urdf.xacro"
    ...
    display_urls() {
        ...
        echo "   Username: Apptronik"
        echo "   Password: Apollo"
        echo ""
...
run_secure_web_control() {
    ...
    # Create htpasswd file with credentials: Apptronik/Apollo
    echo "Creating password file..."
    if ! sudo htpasswd -cb "$HTPASSWD_FILE" "Apptronik" "Apollo" 2>/dev/null; then
        echo "❌ Failed to create password file"
        exit 1
    fi
    sudo chmod 644 "$HTPASSWD_FILE"
    echo "✅ Authentication configured (Username: Apptronik)"

    # Deploy nginx configuration
    echo "📋 Deploying nginx configuration..."
    sudo cp "$WORKSPACE_DIR/nginx_cori.conf" /etc/nginx/sites-available/cori
